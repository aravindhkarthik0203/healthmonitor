...
public class DoctorMainFormController implements Initializable {
    ...
    @FXML
    private TextField PHS_bloodglucose;

    @FXML
    private TextField PHS_bloodpresure;

    @FXML
    private TextField PHS_bmi;

    @FXML
    private TextField PHS_bodyweight;

    @FXML
    private Button PHS_clearBtn;

    @FXML
    private TableView<PHSData> PHS_tableView;

    @FXML
    private TableColumn<PHSData, String> PHS_col_appoinmentID;

    @FXML
    private TableColumn<PHSData, String> PHS_col_bloodglucose;

    @FXML
    private TableColumn<PHSData, String> PHS_col_bloodpresure;

    @FXML
    private TableColumn<PHSData, String> PHS_col_bmi;

    @FXML
    private TableColumn<PHSData, String> PHS_col_bodyweight;

    @FXML
    private TableColumn<PHSData, String> PHS_col_date;

    @FXML
    private TableColumn<PHSData, String> PHS_col_date_delete;

    @FXML
    private TableColumn<PHSData, String> PHS_col_date_modify;

    @FXML
    private TableColumn<PHSData, String> PHS_col_gender;

    @FXML
    private TableColumn<PHSData, String> PHS_col_heartrate;

    @FXML
    private TableColumn<PHSData, String> PHS_col_name;

    @FXML
    private TableColumn<PHSData, String> PHS_col_patientID;

    @FXML
    private TableColumn<PHSData, String> PHS_col_status;

    @FXML
    private Button PHS_deleteBtn;

    @FXML
    private ComboBox<String> PHS_gender;

    @FXML
    private TextField PHS_heartrate;

    @FXML
    private Button PHS_insertBtn;

    @FXML
    private TextField PHS_name;

    @FXML
    private TextField PHS_patientID;

    @FXML
    private DatePicker PHS_schedule;

    @FXML
    private ComboBox<String> PHS_status;

    @FXML
    private Button PHS_updateBtn;

    @FXML
    private Button Pheathstatus_btn;

    @FXML
    private AnchorPane Pheathstatus_form;
...
//    DATABASE TOOLSs
    ...
    public void dashboardNOP() {
        ...
        String sql ="SELECT DATE(date) as date, COUNT(id) FROM patient WHERE doctor = '"
                + Data.doctor_id + "' GROUP BY DATE(date) ORDER BY DATE(date) ASC LIMIT 8";
    ...
    public void dashboardNOA() {
        ...
        String sql = "SELECT DATE(date) as date, COUNT(id) FROM appointment WHERE doctor = '"
                + Data.doctor_id + "' GROUP BY DATE(date) ORDER BY DATE(date) ASC LIMIT 7";
        connect = Database.connectDB();
...
// NOW, LETS TRY 
    ...
    public void patientRecordBtn() {
        try {
            ...
            stage.setTitle("Health Monitor | Record of Patients");
...
// TO SELECT THE DATA PER ROW IN THE TABLE
    ...
    public void switchForm(ActionEvent event) {
        if (event.getSource() == dashboard_btn) {
            ...
            Pheathstatus_form.setVisible(false);
        ...
        else if(event.getSource() == Pheathstatus_btn)
        {
            dashboard_form.setVisible(false);
            patients_form.setVisible(false);
            Pheathstatus_form.setVisible(true);
            appointments_form.setVisible(false);
            profile_form.setVisible(false);
    ...
    //Patients Helath Status

    public void PHSInsertBtn()
    {
        try
        {
...
//        CHECK IF THE FIELD(S) ARE EMPTY
            if (patients_patientID.getText().isEmpty()
                    || PHS_name.getText().isEmpty()
                    || PHS_gender.getSelectionModel().getSelectedItem() == null
                    || PHS_bmi.getText().isEmpty()
                    || PHS_bodyweight.getText().isEmpty()
                    || PHS_heartrate.getText().isEmpty()
                    || PHS_bloodpresure.getText().isEmpty()
                    || PHS_bloodglucose.getText().isEmpty()
                    || PHS_status.getSelectionModel().getSelectedItem() == null
                    || PHS_schedule == null)
            {
//            alert.errorMessage("Please fill the blank fields");
            }
...
//                result = statement.executeQuery(checkPatientID);
                    ...
                    if (result.next()) {
//                    alert.errorMessage(patients_patientID.getText() + " was already taken");
                        alert.successMessage("asdnfjsdfnskdfjsdf");
                    } else {
                        String getSpecialized = "";
                        String getDoctorData = "SELECT * FROM doctor WHERE doctor_id = '"
                                + Data.doctor_id + "'";
                        ...
                        String insertData = "INSERT INTO healthtracking (patient_ID, name, gender"
                                + ", bmi, b_weight, blood_presure, blood_gulcouse"
                                + ",heart_rate,date, status, doctor, specialized, schedule) "
                                + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
                        prepare = connect.prepareStatement(insertData);

                        prepare.setString(1, patients_patientID.getText());
                        prepare.setString(2, PHS_name.getText());
                        prepare.setString(3, (String) PHS_gender.getSelectionModel().getSelectedItem());
                        prepare.setString(4, PHS_bmi.getText());
                        prepare.setString(5, PHS_bodyweight.getText());
                        prepare.setString(6, PHS_heartrate.getText());
                        prepare.setString(7, PHS_bloodpresure.getText());
                        prepare.setString(8, PHS_bloodglucose.getText());

                        java.sql.Date sqlDate = new java.sql.Date(new Date().getTime());

                        prepare.setString(9, "" + sqlDate);
                        prepare.setString(10, (String) PHS_status.getSelectionModel().getSelectedItem());
                        prepare.setString(11, Data.doctor_id);
                        prepare.setString(12, getSpecialized);
                        prepare.setString(13, "" + PHS_schedule.getValue());
...
//                    prepare.executeUpdate();
                        ...
                        if(result.next())
                        {
                            ...
                            alert.errorMessage("Unsuccessfyllt Addes the data");
                        }
...
//                    alert.successMessage("Successully added!");
    ...
    public void patientGetPatientID() {
        ...
        try {
            ...
            result = prepare.executeQuery();
            if (result.next()) {
                tempAppID = result.getInt("MAX(patient_ID)");
            }
            if (tempAppID == 0) {
                tempAppID += 1;
            } else {
    ...
    public void patientsPatientID() {
        patientGetPatientID();

        PHS_patientID.setText("" + patient_ID);

    }

    public ObservableList<PHSData> phsListData;

    public void PHSShowData() {
        phsListData = PHSGetData();

//        PHS_col_appoinmentID.setCellValueFactory(new PropertyValueFactory<>("appointment_id"));
        PHS_col_patientID.setCellValueFactory(new PropertyValueFactory<>("patient_ID"));
        PHS_col_name.setCellValueFactory(new PropertyValueFactory<>("name"));
        PHS_col_gender.setCellValueFactory(new PropertyValueFactory<>("gender"));
        PHS_col_bmi.setCellValueFactory(new PropertyValueFactory<>("bmi"));
        PHS_col_bodyweight.setCellValueFactory(new PropertyValueFactory<>("b_weight"));
        PHS_col_heartrate.setCellValueFactory(new PropertyValueFactory<>("heart_rate"));
        PHS_col_bloodpresure.setCellValueFactory(new PropertyValueFactory<>("blood_presure"));
        PHS_col_bloodglucose.setCellValueFactory(new PropertyValueFactory<>("blood_gulcouse"));
        PHS_col_date.setCellValueFactory(new PropertyValueFactory<>("date"));
        PHS_col_date_modify.setCellValueFactory(new PropertyValueFactory<>("date_modify"));
        PHS_col_date_delete.setCellValueFactory(new PropertyValueFactory<>("date_delete"));
        PHS_col_status.setCellValueFactory(new PropertyValueFactory<>("status"));


        PHS_tableView.setItems(phsListData);
    }

    private ObservableList<PHSData> PHSGetData() {
        ObservableList<PHSData> listData = FXCollections.observableArrayList();

        String sql = "SELECT * FROM healthtracking WHERE date_delete IS NULL and doctor = '"
                + Data.doctor_id + "'";
...
//            Date date, Date dateModify, Date dateDelete, String status, Date schedule
                ...
                appData = new PHSData(result.getInt("patient_ID"),
                        result.getString("name"), result.getString("gender"),
                        result.getDouble("bmi"), result.getDouble("b_weight"),
                        result.getDouble("heart_rate"),result.getDouble("blood_presure"),
                        result.getDouble("blood_gulcouse"), result.getDate("date"),
                        result.getDate("date_modify"), result.getDate("date_delete"),
                        result.getString("status"), result.getDate("schedule"));
                // STORE ALL DATA
                listData.add(appData);
    ...
    public void PHSformSelect()
    {

        PHSData appData = PHS_tableView.getSelectionModel().getSelectedItem();
        int num = PHS_tableView.getSelectionModel().getSelectedIndex();

        if ((num - 1) < -1) {
            return;
        }

        PHS_patientID.setText("" + appData.getPatient_ID());
        PHS_name.setText(appData.getName());
        PHS_gender.getSelectionModel().select(appData.getGender());
        PHS_bmi.setText(""+appData.getB_weight());
        PHS_bmi.setText(""+appData.getBlood_gulcouse());
        PHS_bmi.setText(""+appData.getBlood_presure());
        PHS_bmi.setText(""+appData.getHeart_rate());
        PHS_status.getSelectionModel().select(appData.getStatus());

    }

    public void PHSGenderList() {
        List<String> listG = new ArrayList<>();
        ...
        ObservableList listData = FXCollections.observableArrayList(listG);
        PHS_gender.setItems(listData);

    }

    public void PHSStatusList() {
        List<String> listS = new ArrayList<>();
        ...
        ObservableList listData = FXCollections.observableArrayList(listS);
        PHS_status.setItems(listData);

    }
    public void PHSClearBtn()
    {
        PHS_patientID.clear();
        PHS_name.clear();
        PHS_gender.getSelectionModel().clearSelection();
        PHS_bmi.clear();
        PHS_bodyweight.clear();
        PHS_heartrate.clear();
        PHS_bloodpresure.clear();
        PHS_bloodglucose.clear();
        PHS_status.getSelectionModel().clearSelection();
        PHS_schedule.setValue(null);
    }
